...
async function checkAction(req, res, next) {
    try {
        ...
        if (!description || !notes || !project_id) {
            res.status(400).json({ message: 'Please enter a project_id, description, and notes to submit a new action' });
        } else {
            const possibleProject = await Project.get(project_id);
            if (!possibleProject) {
                next({ status: 404, message: `No project found with ID: ${project_id}` });
            } else {
